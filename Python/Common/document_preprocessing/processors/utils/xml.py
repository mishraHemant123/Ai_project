...
#                             continue
#                         ch = ch.rstrip().lstrip ()
#                         font_info={"font_name": None, "fontsize": None}
...
#                                 font_info["fontsize"]=round(float(word.attrib["size"]))
...
def make_word_box(chars):
    ...
    chars = sorted (chars, key=lambda x: (x[0], x[1]))
    tmp = [c[-1] for c in chars]
    word_box = [chars[0][0], chars[0][1],chars[-1][2], chars[-1][3], chars[0][4], ''.join(tmp)]
    return word_box
...
#from xml.py
def is_y_similar_xml(ry0, ryl, y0, yl):
    if ry0 == y0:
        return True
    if ry0 < y0 < ryl:
        return True
        ...
        for child in figure:
            if child.tag == "text":
                ...
                if "size" in child.attrib:
                    font_info["pointsize"] = round (float(child.attrib["size"]))
                box.append(font_info)
                box.append(child.text)
                chars.append(box)
    chars = sorted(chars, key=lambda x: (x[1], [0]))
    words = []
    fig_inds = []
    for y in unique_y.keys():
        ...
        for idx, c in enumerate(chars):
            if idx not in fig_inds:
                y0 = c[1]
                yl=c[3]
                if is_y_similar_xml(ref_y0, ref_y1, y0, yl):
                    C.append(c)
                    inds.append(idx)
            ...
            if i == 0:
                ...
                if (g >= 2* med) or (len(cc[-1].rstrip().lstrip()) == 0):
                    ...
                    for l in word:
                        tmp.append(l[-1])
                    if len(tmp) > 0:
                        bb =[word[0][0], word[0][1], word [-1][2], word [-1][3], word[0][4], "".join(tmp)]
                        words.append(bb)
                    if g >= 2. * med:
                        word=[cc]
                    else:
...
def extract_image_regions(page_matrix):
    ...
    contours,_= cv2.findContours(page_matrix, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    regions=sorted(regions,key=lambda x: (x[1],[0]))
    return regions
